...
mod tests {
    ...
    fn encode_package_name_handles_scoped_packages() {
        assert_eq!(
            NpmRegistryClient::encode_package_name("@scope/pkg"),
            "%40scope%2fpkg"
        );
    ...
    #[tokio::test]
    async fn fetch_package_parses_scripts_and_deprecated_versions() {
        let mock_server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/%40scope%2fpkg"))
            .respond_with(ResponseTemplate::new(200).set_body_raw(
                r#"{
                  ...
                  "versions": {
                    ...
                    "0.9.0": { "deprecated": "legacy", "scripts": {} }
                  },
                  "time": {
                    "1.0.0": "2024-01-01T00:00:00Z",
                    "0.9.0": "2023-01-01T00:00:00Z"
                  }
                }"#,
                "application/json",
            ))
    ...
    async fn fetch_package_maps_404_to_not_found() {
        ...
        let err = client
            ...
            .await
            .expect_err("missing package should be not found");
    ...
    async fn fetch_package_requires_latest_dist_tag() {
        ...
        let err = client
            .fetch_package("demo")
            .await
            .expect_err("latest dist-tag is required");
    ...
    async fn prefetch_bulk_populates_cache_for_fetch_weekly_downloads() {
        let mock_server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/downloads/point/last-week/demo"))
            .respond_with(ResponseTemplate::new(200).set_body_raw(
                r#"{
                  "downloads": [
                    { "package": "demo", "downloads": 42 }
                  ]
                }"#,
                "application/json",
            ))
        ...
        client
            .prefetch_weekly_downloads_bulk(&["demo".to_string(), "@scope/pkg".to_string()])
            .await
