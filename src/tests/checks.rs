...
fn all_supported_checks() -> Vec<CheckId> {
    check_descriptors()
        .iter()
...
#[tokio::test]
async fn existence_remains_mandatory_when_disabled_in_config() {
    let supported_checks = all_supported_checks();
    let client = FakeRegistryClient {
        result: Err(RegistryError::NotFound {
            registry: "npm",
    ...
    config.checks.disable = vec!["existence".to_string()];
    ...
    assert!(
        report
            .reasons
            .iter()
            .any(|reason| reason.contains("does not exist"))
    );
}
