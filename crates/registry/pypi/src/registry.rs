...
impl PypiRegistryClient {
    pub fn new() -> Self {
        let package_api_base_url = env::var("SAFE_PKGS_PYPI_PACKAGE_API_BASE_URL")
        ...
        // sent to the package-metadata host only, never downloads/popularity hosts).
        let auth_token = token_for_transport(
            &package_api_base_url,
            token_from_env("SAFE_PKGS_PYPI_REGISTRY_TOKEN"),
        );
        Self {
